...
//密码验证
jQuery.validator.addMethod("regexPassword", function(value, element) {return this.optional(element) || /^(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/.test(value);}, "大小写加字符");
//获取时间戳
...
;(function () {
    ...
    var ajaxForm_list = $('form.l_ajaxForm');
    if (ajaxForm_list.length) {
        $('button.l_ajax_submit_btn').on('click', function (e) {
            e.preventDefault();
            var btn = $(this),
                form = btn.parents('form.l_ajaxForm');
            //批量操作 判断选项
            if (btn.data('subcheck')) {
                btn.parent().find('span').remove();
                if (form.find('input.l_check:checked').length) {
