...
class ConfigModel extends Model {
    ...
    public function extendDel($fid) {
        ...
        //删除
        if ($this->where(array('name' => $info['fieldname'], 'groupid' => 2))->delete() !== false) {
    ...
    public function saveExtendConfig($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($this->where(array("name" => $key, 'groupid' => 2))->save($saveData) === false) {
    ...
    public function saveConfig($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            $saveData = array();
            $saveData["value"] = trim($value);
            if ($this->where(array("name" => $key))->save($saveData) === false) {
                $this->error = "更新到{$key}项时，更新失败！";
                return false;
            }
        }
