...
class BehaviorController extends ManageBase {
    ...
    public function index() {
        ...
        $this->basePage('Common/Behavior',$where,array("id" => "desc"));
        //获取总数
       /* $count = $this->behavior->where($where)->count('id');
        $page = new page($count, 2);
        $currentPage = !empty($_REQUEST["pageCurrent"])?$_REQUEST["pageCurrent"]:1;
        $firstRow = ($currentPage - 1) * 2;
        $action = $this->behavior->where($where)->limit($firstRow . ',' . $page->listRows)->order(array("id" => "desc"))->select();
        $this->assign ( 'totalCount', $count )
            ->assign ( 'numPerPage', $page->listRows)
            ->assign ( 'currentPage', $currentPage)
            ->assign("Page", $page->show('Manage'));
