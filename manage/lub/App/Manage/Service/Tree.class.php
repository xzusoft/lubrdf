...
class Tree {
	...
	private function _toFormatTree($list,$level=0,$title = 'title') {
		foreach($list as $key=>$val){
			...
			$val['level'] = $level;
			$val[$title.'_show'] =$level==0?$val[$title]."&nbsp;":$tmp_str.$val[$title]."&nbsp;";
			...
			if(!array_key_exists('_child',$val)){
				...
				unset($val['_child']);
				array_push($this->formatTree,$val);
				Tree::_toFormatTree($tmp_ary,$level+1,$title); //进行下一层递归
			}
		}
		return;
	}
	...
	public function toFormatTree($list,$title = 'title',$pk='id',$pid = 'pid',$root = 0){
		$list = Tree::list_to_tree($list,$pk,$pid,'_child',$root);
		$this->formatTree = array();
		Tree::_toFormatTree($list,0,$title);
