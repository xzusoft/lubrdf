...
class Login extends Item{
	...
	public function tologin($phone = '', $password = '', $code = ''){
    	...
    	if(IS_POST){
			...
			if ($uid > 0) {
				$this->success('登录成功','index/crm/index');
			} else {
				switch ($uid) {
    ...
    public function retrieve($phone = '', $password = '', $code = ''){
    	if(IS_POST){
			...
			$user = model('User');
			if ($user->channge_pwd($password,'',$phone,'2')) {
				return $this->success('密码重置成功!', url('index/login/login'));
			} else {
    ...
    //修改密码
    function changepwd($hpassword = '',$password = ''){
        if(IS_POST){
        	if (!$hpassword || !$password) {
				return $this->error('密码不能为空！');
			}
        	$user = model('User');
        	if($user->channge_pwd($password,$hpassword,'',1)){
        		$user->logout();
        		$this->success('更新成功','index/login/login');
        	}else{
        		$this->error('更新失败...');
        	}
        }else{
