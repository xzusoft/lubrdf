...
{	
	 ...
	 * type  1更新密码2找回密码
	 */
	function channge_pwd($password = '', $hapssword = '', $phone = '', $type = '1'){
		if($type == '1'){
			$uid =  Request::instance()->uinfo->id;
			//获取当前用户信息
			$uinfo = $this->where(['id'=>$uid])->field('verify,password')->find();
			if(!$this->check_pwd($hpassword,$uinfo['verify'],$uinfo['password'])){
				return false;
			}
			$map = array('id'=>$uid);
		}
		...
		$verify = genRandomString();
		//组装新数据数
		return $this->where($map)->update(['verify' => $verify,'password' => hashPassword($password,$verify)]);
	}
	/**
	 * 验证密码
	 */
	function check_pwd($password = '', $verify = '', $md5password = ''){
		if(hashPassword($password,$verify) === $md5password){
